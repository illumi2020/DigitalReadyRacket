...
;positive-nums?: searches to find all positives
;list-of-nodes -> bool
(define (positive-nums? tr)
  (andmap (node-data tr)
          (positive?
           (λ (node base)(positive-nums? node))
           0(node-children tr))))

;(check-expect (positive-nums? list)#f))
...
;Takes count of all the file directorys/paths & its subdirectories from
;a given file then gives back the total
;how-many?: file-directory -> num

(define (how-much directory)
  (abs (node-data directory)
  (foldr
   (λ (node base)(abs(how-much node)base))
   0(node-children directory))))
    

(check-expect(how-much "Program Files")1)

#| FIRST_ATTEMPT
(define (how-many tr)
  (abs (node-data tr)
         (foldr
         (λ (node base)(+(how-many node)base)
         0 (node-children tr)))))
|#

#|
 (check-expect(how-many "Program Files")?)
 (check-expect(how-many "Program Files")0)
|#
...
(define (rename target NN dir)
  (cond
    [(file? dir) (string=? (file-name dir))]
    [(folder? dir)
     (ormap (λ (fol) (target NN fol))
     (folder-directories dir))]))


    ;[(if (not(string=? NN file) "Target File Not Found"